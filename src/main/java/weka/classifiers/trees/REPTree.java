...
public class REPTree extends AbstractClassifier implements OptionHandler,
	...
	protected class Tree implements Serializable, RevisionHandler {
		...
		/** The M value of M-estimate smoothing. */
		protected int m_M = 4;
		...
		/**
		 * The (unnormalized) class distribution in the nominal case. Holds the
		 * sum of squared errors and the weight in the numeric case.
		 */
		protected double[] m_Distribution = null;

		/**
		 * Class distribution of hold-out set at node in the nominal case.
		 * Straight sum of weights in the numeric case (i.e. array has only one
		 * element.
		 */
			...
			if (m_Attribute > -1) {
				...
				if (instance.isMissing(m_Attribute)) {
					...
					returnedDist = m_Successors[(int) instance
							.value(m_Attribute)].distributionForInstance(
							instance, prob);
				} else {
					// For numeric attributes
					if (instance.value(m_Attribute) < m_SplitPoint) {
						returnedDist = m_Successors[0].distributionForInstance(
								instance, prob);
					} else {
						returnedDist = m_Successors[1].distributionForInstance(
								instance, prob);
					}
				}
			}
		 ...
		 */
		public double getHeight(Instance instance) {
			if (m_Attribute == -1) {
				return 1;
			} else {
				double h;
				if (instance.isMissing(m_Attribute)) {
					...
					h = m_Successors[1].getHeight(instance);
					h++;
				}
				return h;
			}
		 ...
		 * routine assumes to be called in the order of branching, enabling us
		...
		public final String sourceExpression(int index) {
			...
			if (m_Info.attribute(m_Attribute).isNominal()) {
				...
				expr.append(".equals(\"").append(
						m_Info.attribute(m_Attribute).value(index)).append(
						"\")");
			} else {
				expr = new StringBuffer("");
				if (index == 0) {
					expr.append("((Double)i[").append(m_Attribute).append(
							"]).doubleValue() < ").append(m_SplitPoint);
				} else {
					expr.append("true");
				}
			}
		 ...
		 * assigned to variable "p", and assumes the tested instance is named
		 * "i". The results are returned as two stringbuffers: a section of code
		 * for assignment of the class, and a section of code containing support
		 * code (eg: other support methods).
		 * <p/>
		 * TODO: If the outputted source code encounters a missing value for the
		 * evaluated attribute, it stops branching and uses the class
		 * distribution of the current node to decide the return value. This is
		 * unlike the behaviour of distributionForInstance().
		 * 
		 * @param className
		 ...
		 *         containing assignment code, and the second containing source
		 *         for support code.
		 * @throws Exception
		...
		public StringBuffer[] toSource(String className, Tree parent)
			...
			if (m_Attribute == -1) {
				...
				text.append("  static double N").append(
						Integer.toHexString(this.hashCode()) + printID).append(
						"(Object []i) {\n").append(
						"    double p = Double.NaN;\n");
				...
				for (int i = 0; i < m_Successors.length; i++) {
					...
					if (m_Successors[i].m_Attribute == -1) {
						...
						StringBuffer[] sub = m_Successors[i].toSource(
								className, this);
						text.append("" + sub[0]);
						atEnd.append("" + sub[1]);
					}
		...
		protected String leafString(Tree parent) throws Exception {
			...
			if (m_Info.classAttribute().isNumeric()) {
				...
				return " : "
						...
						+ Utils
								.doubleToString(
										(Utils.sum(m_Distribution) - m_Distribution[maxIndex]),
										2)
						+ ")"
						+ " ["
						...
						+ Utils
								.doubleToString(
										(Utils.sum(m_HoldOutDist) - m_HoldOutDist[maxIndex]),
										2) + "]";
			}
		}
		...
		protected void buildTree(int[][] sortedIndices, double[][] weights,
			...
			// Implementing Laplace correction <----
			if (m_Smoothing == SMOOTHING_LAPLACE) {
				for (int i = 0; i < m_ClassProbs.length; i++) {
					m_ClassProbs[i]++;
				}
			}

			// Implementing M-estimate smoothing <-------
			if (m_Smoothing == SMOOTHING_M_ESTIMATE) {
				for (int i = 0; i < m_ClassProbs.length; i++) {
					m_ClassProbs[i] += m_M * 1.0 / data.numClasses();
				}
			}
		...
		protected double distribution(double[][] props, double[][][] dists,
				int att, int[] sortedIndices, double[] weights,
				double[][] subsetWeights, Instances data) throws Exception {
		...
		protected double numericDistribution(double[][] props,
				double[][][] dists, int att, int[] sortedIndices,
				double[] weights, double[][] subsetWeights, Instances data,
				double[] vals) throws Exception {
	...
	/** no smoothing. */
	public static final int SMOOTHING_NONE = 1;
	/** Laplace correction. */
	public static final int SMOOTHING_LAPLACE = 2;
	/** M-Estimate. */
	 ...
	 */
	public SelectedTag getSmoothing() {
		return new SelectedTag(m_Smoothing, TAGS_SMOOTHING);
	}
	 ...
	 */
	public void setSmoothing(SelectedTag newMethod) {
	...
	public Enumeration listOptions() {
		...
		newVector.addElement(new Option(
				"\tSet minimum numeric class variance proportion\n"
						+ "\tof train variance for split (default 1e-3).", "V",
				1, "-V <minimum variance for split>"));
		newVector
				.addElement(new Option(
						...
						"-N <number of folds>"));
		newVector.addElement(new Option(
				"\tSeed for random data shuffling (default 1).", "S", 1,
				"-S <seed>"));
	...
	public void setOptions(String[] options) throws Exception {
		...
		if (Utils.getFlag('A', options)) {
			setSmoothing(new SelectedTag(SMOOTHING_LAPLACE, TAGS_SMOOTHING));
